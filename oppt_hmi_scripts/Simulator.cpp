...
#include "solvers/ABT/robotModel/RobotObservation.hpp"
...
{   
    ...
    oppt::ActionSharedPtr actionPtr = static_cast<robot::DiscreteRobotAction const&>(*action).getOpptAction();
    oppt::VectorFloat actionVec = actionPtr->as<oppt::VectorAction>()->asVector();
    std::string actionStr = "echo -n " + std::to_string(actionVec[0]) + "," + std::to_string(actionVec[1]) + " > " + oppt::hmi::PIPE_PATH_TO_GAMA;
    cout << "Sent off action " + actionStr << endl;
    FILE * pipeToGama(popen(actionStr.c_str(), "w"));
    pclose(pipeToGama);
    std::string cmd = "cat < " + oppt::hmi::PIPE_PATH_TO_SOLVER;
    std::string resultCmd = oppt::hmi::execute(cmd.c_str());
    cout << "Received state " + resultCmd << endl;
    abt::Model::StepResult result;
    oppt::VectorFloat nextStateVec;
    oppt::VectorFloat observationVec;
    std::string nextStateString = resultCmd.substr(0, resultCmd.find("\n"));
    std::string observationString = resultCmd.substr(resultCmd.find("\n") + 1);
    while (!nextStateString.empty()) {
        nextStateVec.push_back(std::stof(nextStateString.substr(0, nextStateString.find(","))));
	    nextStateString = nextStateString.substr(nextStateString.find(",") + 1);
    }
    while (!observationVec.empty()) {
        observationVec.push_back(std::stof(observationString.substr(0, observationString.find(","))));
        observationString = observationString.substr(observationString.find(",") + 1);
    }
    robot::DiscreteRobotAction robotAction = static_cast<robot::DiscreteRobotAction const&>(*action);
    result.action = robotAction.copy();
    oppt::ObservationSharedPtr observationPtr = std::make_shared<oppt::DiscreteVectorObservation>(observationVec);
    shared::RobotObservation robotObservation(observationPtr);
    result.observation = robotObservation.copy();
    oppt::RobotStateSharedPtr nextState = std::make_shared<oppt::VectorState>(nextStateVec);
    shared::RobotState nextRobotState(nextState);
    result.reward = model_->generateReward(*currentState, *robotAction.copy(), nullptr, nextRobotState.copy().get());
    result.isTerminal = model_->isTerminal(*currentState);
    result.nextState = nextRobotState.copy();
